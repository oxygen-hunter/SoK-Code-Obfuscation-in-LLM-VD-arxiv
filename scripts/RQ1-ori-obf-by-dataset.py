...
# 定义模型排序顺序
SERIES_ORDER = ["qwen", "llama", "deepseek", "openai"]

datasets_macro = {
    "smartbugs": "\\smartbugs",
    "PrimeVul_c": "\\primevulc",
    "ReposVul_cpp": "\\reposvulcpp",
    "ReposVul_py": "\\reposvulpy"
}
...
def output_all_datasets_to_latex(datasets, dfs_out, output_dir):
    ...
    latex_lines.append("\\centering")
    latex_lines.append("\\caption{Detection successful rate on different datasets: \\smartbugs, \\reposvulcpp, \\reposvulpy, \\primevulc}")
    latex_lines.append("{")
    latex_lines.append("\\scriptsize")
    latex_lines.append("\\setlength\\tabcolsep{3pt}")
    latex_lines.append("\\renewcommand{\\arraystretch}{0.8}")
    latex_lines.append("\\begin{tabular}{l lccccccccccccccc}")
    latex_lines.append("\\toprule")
    latex_lines.append("dataset & series & \\multicolumn{6}{c}{qwen} & \\multicolumn{4}{c}{llama} & \\multicolumn{2}{c}{deepseek} & \\multicolumn{3}{c}{openai} \\\\")
    latex_lines.append("\\cmidrule(lr){3-8} \\cmidrule(lr){9-12} \\cmidrule(lr){13-14} \\cmidrule(lr){15-17}")
    latex_lines.append(" & model & qn-7b & qn-14b & qn-32b & ds-qn-7b & ds-qn-14b & ds-qn-32b & lm-8b & lm-70b & ds-lm-8b & ds-lm-70b & ds-v3 & ds-r1 & gpt-3.5 & gpt-4o & o3-mini \\\\")
    ...
    for dataset, df_out in zip(datasets, dfs_out):
        ...
        for _, row in df_out.iterrows():
            ...
            latex_lines.append(" & ".join(values) + " \\\\")
            if combo_name == "original":
                latex_lines.append("\\cmidrule(lr){2-17}")
        latex_lines.append(" ")
    ...
    tex_path = os.path.join(output_dir, f"all_datasets_detection_result.tex")
    with open(tex_path, "w", encoding="utf-8") as f:
        f.write("\n".join(latex_lines))
...
if __name__ == "__main__":
    for eval_mode in ["exist", "type"]:
        output_dir = os.path.join("RQs/RQ1/detection_result_by_dataset", eval_mode)
